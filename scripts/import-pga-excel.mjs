...
function parseExcelFile(filePath) {
  ...
  for (let i = 2; i < data.length; i++) {
    ...
    const entry = {
      ...
      salvationsLivestreamEnc: 0,
      salvationsLivestreamYxp: 0,
      salvationsInhouse: 0,
      salvationsMc: 0,
      salvationsOther: 0,
      baptisms: 0,
...
async function importEntries(matchedEntries, existingEntries, reportId) {
  ...
  for (const entry of matchedEntries) {
    const entryData = {
      ...
      hc1: entry.hc1,
      hc2: entry.hc2,
      salvations_livestream_enc: entry.salvationsLivestreamEnc,
      salvations_livestream_yxp: entry.salvationsLivestreamYxp,
      salvations_inhouse: entry.salvationsInhouse,
      salvations_mc: entry.salvationsMc,
