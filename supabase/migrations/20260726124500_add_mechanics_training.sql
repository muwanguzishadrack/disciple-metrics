-- Add "Mechanics Training" as a standalone Others metric, alongside Baptisms and MCA.
--
-- Deliberately NOT part of the generated `mechanics` total: that stays the sum of the
-- seven serving teams (plus mechanics_legacy). Mechanics Training counts people trained,
-- not people serving on the day, so folding it in would inflate the mechanics figure.
-- The name only shares the `mechanics_` prefix; the generated expression is explicit
-- and does not include this column.

alter table public.pga_entries
  add column mechanics_training integer default 0;

-- Recreate the views so pga_report_summary exposes the new sum
