...
import { useMechanicsSummary, useDeletePgaReport, type MechanicsSummaryRow } from '@/hooks/use-pga'
...
interface MechanicsReportTabProps {
  actionsContainer?: HTMLDivElement | null
}

export function MechanicsReportTab({ actionsContainer }: MechanicsReportTabProps) {
  const router = useRouter()
  const { toast } = useToast()
  const { data: userRole } = useUserRole()
  const isAdmin = userRole === 'admin'
  const { data: summaryRows = [], isLoading } = useMechanicsSummary()
  ...
  const [deleteTarget, setDeleteTarget] = useState<MechanicsSummaryRow | null>(null)
  ...
  const handleExport = () => {
    exportToExcel<MechanicsSummaryRow>({
      data: filteredRows,
      columns: [
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => router.push(`/reports/mechanics/${row.date}`)}>
                            View
                          </DropdownMenuItem>
                          {isAdmin && (
                            <DropdownMenuItem
