...
import { useFourWeekPgaSummary, useDeletePgaReport, type FourWeekPgaSummaryRow } from '@/hooks/use-pga'
...
interface FourWeekPgaTabProps {
  actionsContainer?: HTMLDivElement | null
}

export function FourWeekPgaTab({ actionsContainer }: FourWeekPgaTabProps) {
  const router = useRouter()
  const { toast } = useToast()
  const { data: userRole } = useUserRole()
  const isAdmin = userRole === 'admin'
  const { data: summaryRows = [], isLoading } = useFourWeekPgaSummary()
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => router.push(`/reports/4-week-pga/${row.date}`)}>
                            View
                          </DropdownMenuItem>
                          {isAdmin && (
                            <DropdownMenuItem
