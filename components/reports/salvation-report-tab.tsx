...
import { useSalvationSummary, useDeletePgaReport, type SalvationSummaryRow } from '@/hooks/use-pga'
...
interface SalvationReportTabProps {
  actionsContainer?: HTMLDivElement | null
}

export function SalvationReportTab({ actionsContainer }: SalvationReportTabProps) {
  const router = useRouter()
  const { toast } = useToast()
  const { data: userRole } = useUserRole()
  const isAdmin = userRole === 'admin'
  const { data: summaryRows = [], isLoading } = useSalvationSummary()
  ...
  const [deleteTarget, setDeleteTarget] = useState<SalvationSummaryRow | null>(null)
  ...
  const handleExport = () => {
    exportToExcel<SalvationSummaryRow>({
      data: filteredRows,
      columns: [
        ...
        { header: 'Salv-Total', accessor: 'total' },
      ],
      sheetName: 'Salvation Report',
      fileName: 'salvation_report',
      includeTotals: true,
    })
  }
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Salv-Total</TableHead>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => router.push(`/reports/salvation/${row.date}`)}>
                            View
                          </DropdownMenuItem>
                          {isAdmin && (
                            <DropdownMenuItem
