...
import { ArrowLeft, Clock, Sparkles, Baby, Globe, Building, TrendingUp, ChevronFirst, ChevronLast, ChevronLeft, ChevronRight, Download } from 'lucide-react'
...
import { useFourWeekPgaDetail, type FourWeekRow } from '@/hooks/use-pga'
...
export default function FourWeekPgaDetailPage() {
  const params = useParams()
  const router = useRouter()
  const reportDate = params.date as string
  const { data, dates, metricAverages, isLoading } = useFourWeekPgaDetail(reportDate)
  ...
  const handleExport = () => {
    ...
    exportToExcel<FourWeekRow>({
      data,
      columns,
      sheetName: '4 Week PGA Average',
      fileName: `4_week_pga_average_${reportDate}`,
      includeTotals: true,
    })
  }
  ...
  const stats = [
    ...
    { title: 'Kids Avg', value: metricAverages.kids, icon: Baby },
    { title: 'Local Avg', value: metricAverages.local, icon: Globe },
    { title: 'HC1 Avg', value: metricAverages.hc1, icon: Building },
    { title: 'HC2 Avg', value: metricAverages.hc2, icon: Building },
    { title: 'Overall Avg', value: metricAverages.total, icon: TrendingUp },
  ]
  ...
  return (
    <div>
      <PageHeader
        title={formattedDate}
        description="4 Week PGA Average Details"
        actions={
          <Button
            variant="outline"
            onClick={() => router.back()}
