...
import { useSalvationReport, type SalvationRow } from '@/hooks/use-pga'
...
export default function SalvationDetailPage() {
  const params = useParams()
  const router = useRouter()
  const reportDate = params.date as string
  const { data = [], isLoading } = useSalvationReport(reportDate)
  ...
  const handleExport = () => {
    exportToExcel<SalvationRow>({
      ...
      fileName: `salvation_report_${reportDate}`,
      includeTotals: true,
    })
  }
  ...
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <p className="text-muted-foreground">No salvation data found for {reportDate}</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <PageHeader
        title={formattedDate}
        description="Salvation Report Details"
        actions={
          <Button
            variant="outline"
            onClick={() => router.back()}
